...
$(function() {
  $("li").on("click",function() {
	  if(this.id=="dnt") {
		  $("#dnt_txt").html("You can donate USD via PayPal mail: elhizazi1 "+"@"+"icloud.com");
	  }
  });
});
function loadPackageInfo() {
	...
	if (navigator.userAgent.search(/Cydia/) == -1) {
		$("#showAddRepo_").show();
		$("#open_cydia").attr("href","cydia://url/https://cydia.saurik.com/api/share#?source=https://apt.neutronfile.org/&package="+urlSelfParts[1]);
	}
	$.ajax({
		url: form_url,
		type: "GET",
...
{
	var sectionContent = "";
	sectionContent += "<li class=\"has-icon\"><a onclick=\"loadMainSection()\" role=\"button\"><img style=\"border-radius: 20%;\" href=\"imgpack/back.png\" alt=\"\" srcset=\"imgpack/back.png 2x, imgpack/back.png 3x\" class=\"icon\"/><label><< Back</label></a></li>";
	for (var keyNow in packagesSection[sectionName]) {
		...
		sectionContent +=  "<li class=\"has-icon\"><a href='"+urlOpen+"' target='_blank' role=\"button\"><img style=\"border-radius: 20%;\" href=\"imgpack/"+encodeURI(sectionName)+".png\" alt=\"\" srcset=\"imgpack/"+encodeURI(sectionName)+".png 2x, imgpack/"+encodeURI(sectionName)+".png 3x\" class=\"icon\"/><label>"+dicNow.name+" v"+dicNow.version+"</label></a></li>";
	}
...
function loadMainSection()
{
	...
	for (var section in packagesSection) {		
		sectionContent += "<li class=\"has-icon\"><a onclick=\"openSection('"+section+"')\" role=\"button\"><img style=\"border-radius: 20%;\" href=\"imgpack/"+encodeURI(section)+".png\" alt=\"\" srcset=\"imgpack/"+encodeURI(section)+".png 2x, imgpack/"+encodeURI(section)+".png 3x\" class=\"icon\"/><label>"+section+" ("+packagesSection[section].length+")</label></a></li>";
	}
	$("#browser").html(sectionContent);
}
...
function loadRecentUpdates()
{
	...
	for (var dicNow in allPackages) {
		...
		htmlnews +=  "<li class=\"has-icon\"><a href='"+urlOpen+"' target='_blank' role=\"button\"><img style=\"border-radius: 20%;\" href=\"imgpack/"+encodeURI(allPackages[dicNow].section)+".png\" alt=\"\" srcset=\"imgpack/"+encodeURI(allPackages[dicNow].section)+".png 2x, imgpack/"+encodeURI(allPackages[dicNow].section)+".png 3x\" class=\"icon\"/><label>"+allPackages[dicNow].name+" v"+allPackages[dicNow].version+"</label></a></li>";
	}
