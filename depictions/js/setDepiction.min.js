        ...
        $.ajax({
            ...
            console.log("Bắt đầu phân tích cú pháp XML."), $(n).find("packageInfo").each(function() {
                ...
                var t = $(this).find("changeVersion:first").text().replace("v", "").trim(),
                    [t] = (compatible($(this).find("miniOS").text().trim(), $(this).find("maxiOS").text().trim()), $(n).find("description").each(function() {
            ...
            var t = 'Package "' + o + '" not found. Incorrect parameter or no depiction for this package.';
            $("#compatibility").append("Unknown"), $("#description").append("<li>" + t + "</li>"), $("#pill").append("v?"), $("#changelog-date").append("../../...."), $("#changelog").append("Unavailable"), $("#dependencies").append("Error"), $("#screenshots").append("No screenshot"), $("#infoTable").append("No info"), $("#links").append("Unavailable")
        })
    } else console.log("Package not found. Aborting.")
...
function lastUpdateDate(o) {
    ...
    return new Promise((r, e) => {
        if (o in updateDatesDict) console.log("Using cached value for url " + o + " (" + updateDatesDict[o] + ")"), r(updateDatesDict[o]);
        else {
            let a = 0;
            const i = () => {
                t(o).then(n => {
                    t(String(o).replace("mmits?path=", "ntents/").concat("?ref=", JSON.parse(n)[a].sha)).then(t => {
                        ...
                        console.log("Date successfully fetched for url: " + o + " (" + e + ")");
                        ...
                        a++, console.warn("Retrying call with commit " + a + " for url " + o), i()
                    })
                }).catch(t => {
                    e("Error getting sha value (" + t + ")")
                })
            };
            i()
        }
